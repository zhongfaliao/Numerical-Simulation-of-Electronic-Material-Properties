function props = ingaas_elec_prop(x, Na, Nd, T)

% x is the concentration of Gallium in the compound semicouductor
% main reference http://www.ioffe.rssi.ru/SVA/NSM/Semicond/GaInAs/index.html

loadconstants;
kbeV = kb/q; % Boltzmann constant divided by eV
props.eps_r_static = 15.1-2.87*x+0.67*x^2; % Dielectric constant (static)
props.eps_r_highf = 12.3-1.4*x; % Dielectric constant (high frequency)

props.meff_e = 0.023+0.037*x+0.003*x^2; % effective electron mass
props.meff_h = 0.41+0.1*x; % effective (heavy) hole mass
props.meff_lh = 0.026+0.056*x; % effective (light) hole mass
